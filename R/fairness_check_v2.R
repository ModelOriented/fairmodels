...
fairness_check <- function(x,
  ...
  cat("Privileged subgroup:\t", color_codes$yellow_start, privileged, color_codes$yellow_end, "\n")
  ...
  ###############  error handling  ###############
  if (! privileged %in% protected) stop("privileged subgroup is not in protected variable vector")

  if(is.null(epsilon)) epsilon <- 0.1
  if(! is.numeric(epsilon) | length(epsilon) > 1) stop("Epsilon must be single, numeric value")
  ...
  if (is.null(label)){
    ...
    if (length(label) != n_exp) stop("Number of labels must be equal to number of explainers")
  }
  ...
  if (any(label %in% fobjects_label)) stop("Explainer has the same label as label in fairness_object")


  # cutoff handling- if cutoff is null than 0.5 for all subgroups
  group_levels <- length(unique(protected))
