...
# making model
set.seed(1)
gbm_model <-gbm(salary ~. , data = data, distribution = "bernoulli")
...
```{r}
data_fixed <- disparate_impact_remover(data = data, protected = adult$sex, 
...
uniform_indexes      <- resample(protected = adult$sex,
                                 y = data$salary)
preferential_indexes <- resample(protected = adult$sex,
                                 y = data$salary,
                                 type = "preferential",
                                 probs = probs)
...
set.seed(1)
gbm_model     <- gbm(salary ~. ,
                     data = data[preferential_indexes,],
                     distribution = "bernoulli")
