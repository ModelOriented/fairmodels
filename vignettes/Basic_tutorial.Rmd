...
fobject <- fairness_check(rf_explainer,                         # explainer
                          protected = compas$Two_yr_Recidivism, # protected variable as factor
...
As we can see it is more likely that model will categorize African Americans as future recidivists than for example Asians. 
But maybe some groups are statistically more likely to go do crimes in the future. It is possible, but there are more ways that the model can discriminate people. 
...
fobject_with_cutoff <- create_fairness_object(explainer_2,explainer_5, explainer_4,
                                  ...
                                  base = "Female", 
                                  cutoff = c(0.63,0.5), 
                                  label = c("gbm_c","lm_c","ranger_c")) # c as in changed  


p1 <- performance_and_fairness(fobject_without_cutoff,
                                    fairness_metric = "TPR_parity_loss",
                                    performance_metric = "accuracy")
p2 <- performance_and_fairness(fobject_with_cutoff,
                                    fairness_metric = "TPR_parity_loss",
                                    performance_metric = "accuracy")

# plot more than one object simply passing objects as arguments
plot(p1,p2)
